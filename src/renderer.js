...
let mrKrabsEl = document.getElementById("mr-krabs");
let pendingNext = null;
let currentDisplayedSvg = getObjectSvgName(mrKrabsEl);
...
window.electronAPI.onStateChange((state, svg) => {
  ...
  if (mrKrabsEl && mrKrabsEl.isConnected && currentDisplayedSvg === svg) {
    if (shouldTrackEyes(state, svg) && !eyeTarget) {
      attachEyeTracking(mrKrabsEl);
...
window.electronAPI.onEyeMove((dx, dy) => {
  ...
  if (eyeTarget && !eyeTarget.ownerDocument?.defaultView) {
    ...
    bodyTarget = null;
    shadowTarget = null;
    if (mrKrabsEl && mrKrabsEl.isConnected) attachEyeTracking(mrKrabsEl);
    return;
  }
  applyEyeMove(dx, dy);
});
...
window.electronAPI.onWakeFromDoze(() => {
  if (mrKrabsEl && mrKrabsEl.contentDocument) {
    try {
      const eyes = mrKrabsEl.contentDocument.getElementById("eyes-doze");
