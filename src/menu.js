...
const i18n = {
  en: {
    ...
    updateAvailableMacMsg: "v{version} is available. Auto-update will download and install it automatically, then restart the app.",
    updateNotAvailable: "You're Up to Date",
    updateNotAvailableMsg: "Mr. Krabs v{version} is the latest version.",
    ...
    restartLater: "Later",
    download: "Download",
    autoUpdate: "Auto Update & Restart",
    ...
    taskListSubmenu: "Task List",
    quickAddTask: "Quick Add Task",
    runTasksNow: "Run Tasks Now",
    proposeTasksNow: "Generate Task Proposals Now",
    sendTarget: "Quick Send (Ctrl+Enter)",
    sendTargetAuto: "Auto (detect best)",
    sendTargetDesktop: "Claude Desktop App",
    sendTargetCode: "Claude Code (Terminal)",
    sendTargetCli: "Claude CLI (terminal injection)",
  ...
  zh: {
    ...
    updateAvailableMacMsg: "v{version} 已发布！选择「自动更新并重启」将自动下载并安装，完成后重启 App。",
    updateNotAvailable: "已是最新版本",
    updateNotAvailableMsg: "Mr. Krabs v{version} 已是最新版本。",
    ...
    sessionHrAgo: "{n}小时前",
    taskPanel: "任务面板",
    taskListSubmenu: "任务清单",
    quickAddTask: "快速添加任务",
    runTasksNow: "立即执行任务",
    proposeTasksNow: "立即生成任务提议",
    sendTarget: "发送目标 (Ctrl+Enter)",
    sendTargetAuto: "自动（优先检测）",
    sendTargetDesktop: "Claude Desktop App",
    sendTargetCode: "Claude Code（终端）",
    sendTargetCli: "Claude CLI（终端注入）",
...
module.exports = function initMenu(ctx) {
  ...
  function createTray() {
    ...
    ctx.tray.setToolTip("Mr. Krabs Desktop Pet");
    buildTrayMenu();
  }
  ...
  function buildTrayMenu() {
      ...
      },
      {
        ...
        click: (menuItem) => {
          ...
          try {
            ...
            console.warn("Mr. Krabs: failed to toggle auto-start hook:", err.message);
          }
          ctx.savePrefs();
        },
      },
    ...
    items.push(
      ...
      { type: "separator" },
      {
        label: t("sendTarget"),
        submenu: [
          { label: t("sendTargetAuto"),    type: "radio", checked: ctx.quickSendMode === "auto",
            click: () => { ctx.quickSendMode = "auto";            buildContextMenu(); buildTrayMenu(); ctx.savePrefs(); } },
          { label: t("sendTargetDesktop"), type: "radio", checked: ctx.quickSendMode === "claude-desktop",
            click: () => { ctx.quickSendMode = "claude-desktop";  buildContextMenu(); buildTrayMenu(); ctx.savePrefs(); } },
          { label: t("sendTargetCode"),    type: "radio", checked: ctx.quickSendMode === "claude-code",
            click: () => { ctx.quickSendMode = "claude-code";     buildContextMenu(); buildTrayMenu(); ctx.savePrefs(); } },
          { label: t("sendTargetCli"),     type: "radio", checked: ctx.quickSendMode === "claude-cli",
            click: () => { ctx.quickSendMode = "claude-cli";      buildContextMenu(); buildTrayMenu(); ctx.savePrefs(); } },
        ],
      },
      { type: "separator" },
      { label: t("shortcutSettings"), click: () => ctx.openShortcutSettingsWindow && ctx.openShortcutSettingsWindow() },
      { type: "separator" },
  ...
  function popupMenuAt(menu) {
    if (ctx.menuOpen) {
      // Safety: force-reset if stuck for more than 5 seconds
      console.warn("Mr. Krabs Menu: menuOpen is true, forcing reset");
      ctx.menuOpen = false;
    }
    ...
    ctx.menuOpen = true;
    // Safety timeout: auto-reset menuOpen if callback never fires
    const menuSafetyTimer = setTimeout(() => {
      if (ctx.menuOpen) {
        console.warn("Mr. Krabs Menu: popup callback not fired after 10s, force-resetting menuOpen");
        ctx.menuOpen = false;
  ...
  function buildContextMenu() {
    const template = [
      ...
      { type: "separator" },
      { label: t("taskPanel"), click: () => ctx.openTaskPanel() },
      {
        label: t("taskListSubmenu"),
        submenu: [
          { label: t("quickAddTask"), click: () => ctx.openQuickTask() },
          { label: t("runTasksNow"), click: () => ctx.startIdleTask() },
          { type: "separator" },
          { label: t("proposeTasksNow"), click: () => ctx.triggerProposals && ctx.triggerProposals() },
        ],
      },
      { type: "separator" },
      {
