...
function syncMrKrabsHooks() {
  try {
    ...
    if (added > 0 || updated > 0 || removed > 0) {
      console.log(`Mr. Krabs: synced hooks (added ${added}, updated ${updated}, removed ${removed})`);
    }
  } catch (err) {
    console.warn("Mr. Krabs: failed to sync hooks:", err.message);
  }
}

function sendStateHealthResponse(res) {
  const body = JSON.stringify({ ok: true, app: MR_KRABS_SERVER_ID, port: getHookServerPort() });
  res.writeHead(200, {
    "Content-Type": "application/json",
    [MR_KRABS_SERVER_HEADER]: MR_KRABS_SERVER_ID,
  });
  res.end(body);
}
...
function startHttpServer() {
    ...
    if (req.method === "GET" && req.url === "/state") {
      ...
      req.on("end", () => {
        ...
        try {
          ...
          if (ctx.STATE_SVGS[state]) {
            ...
            res.writeHead(200, { [MR_KRABS_SERVER_HEADER]: MR_KRABS_SERVER_ID });
            res.end("ok");
          } else {
            res.writeHead(400);
      ...
      req.on("end", () => {
        if (tooLarge) {
          ...
          ctx.sendPermissionResponse(res, "deny", "Permission request too large for Mr. Krabs bubble; answer in terminal");
          return;
        }

        if (ctx.doNotDisturb) {
          ctx.permLog("SKIPPED: DND mode");
          ctx.sendPermissionResponse(res, "deny", "Mr. Krabs is in Do Not Disturb mode");
          return;
        }
  ...
  httpServer.on("listening", () => {
    ...
    console.log(`Mr. Krabs state server listening on 127.0.0.1:${activeServerPort}`);
    syncMrKrabsHooks();
  });
...
return { startHttpServer, getHookServerPort, syncMrKrabsHooks, cleanup };
