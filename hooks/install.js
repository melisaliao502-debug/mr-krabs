#!/usr/bin/env node
// Mr. Krabs Desktop Pet — Hook Installer
...
/**
 * Register Mr. Krabs hooks into ~/.claude/settings.json.
...
function registerHooks(options = {}) {
  ...
  let hookScript = path.resolve(__dirname, "mr-krabs-hook.js").replace(/\\/g, "/");
  ...
  if (options.autoStart) {
    ...
    if (!autoStartSync.found) {
      // Insert at index 0 — must run BEFORE mr-krabs-hook.js so the app is starting
  ...
  if (!options.silent) {
    ...
    console.log(`Mr. Krabs hooks installed to ${settingsPath}`);
