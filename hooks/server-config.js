...
const MR_KRABS_SERVER_ID = "mr-krabs";
const MR_KRABS_SERVER_HEADER = "x-mr-krabs-server";
...
const RUNTIME_CONFIG_PATH = path.join(os.homedir(), ".mr-krabs", "runtime.json");
...
function writeRuntimeConfig(port) {
  ...
  const body = JSON.stringify({ app: MR_KRABS_SERVER_ID, port: safePort }, null, 2);
...
function isMrKrabsResponse(res, body) {
  if (readHeader(res, MR_KRABS_SERVER_HEADER) === MR_KRABS_SERVER_ID) return true;
  if (!body) return false;
  try {
    const data = JSON.parse(body);
    return data && data.app === MR_KRABS_SERVER_ID;
  } catch {
    return false;
  }
}
...
function probePort(port, timeoutMs, callback, options = {}) {
  ...
  const req = httpGet(
    ...
    (res) => {
      ...
      res.on("end", () => callback(isMrKrabsResponse(res, body)));
    }
  );
...
function postStateToPort(port, payload, timeoutMs, callback, options = {}) {
  ...
  const req = httpRequest(
    ...
    (res) => {
      if (readHeader(res, MR_KRABS_SERVER_HEADER) === MR_KRABS_SERVER_ID) {
        res.resume();
        callback(true, port);
        return;
      }
      ...
      res.on("end", () => callback(isMrKrabsResponse(res, responseBody), port));
    }
  );
...
function discoverMrKrabsPort(options, callback) {
