...
function makeTempSettings(initialSettings = {}) {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "mr-krabs-install-"));
...
function getMrKrabsCommands(settings, event) {
  const entries = settings.hooks?.[event];
  if (!Array.isArray(entries)) return [];
  const commands = [];
  for (const entry of entries) {
    if (!entry || typeof entry !== "object") continue;
    if (typeof entry.command === "string" && entry.command.includes("mr-krabs-hook.js")) {
      commands.push(entry.command);
    }
    if (!Array.isArray(entry.hooks)) continue;
    for (const hook of entry.hooks) {
      if (hook && typeof hook.command === "string" && hook.command.includes("mr-krabs-hook.js")) {
...
describe("Hook installer version compatibility", () => {
  it("registers StopFailure when Claude Code is >= 2.1.78", () => {
    ...
    assert.deepStrictEqual(getMrKrabsCommands(settings, "StopFailure").length, 1);
  ...
  it("removes stale Mr. Krabs StopFailure hooks while preserving third-party entries when version is known too old", () => {
    const settingsPath = makeTempSettings({
      hooks: {
        StopFailure: [
          {
            matcher: "",
            hooks: [{ type: "command", command: 'node "/tmp/mr-krabs-hook.js" StopFailure' }],
          },
        ],
        PostCompact: [],
        PreCompact: [
  ...
  it("keeps existing versioned hooks when Claude Code version is unknown", () => {
    ...
    const settings = readSettings(settingsPath);
    assert.ok(Array.isArray(settings.hooks.StopFailure));
    assert.strictEqual(getMrKrabsCommands(settings, "StopFailure").length, 1);
  ...
  it("updates stale hook paths when command marker already exists", () => {
    const settingsPath = makeTempSettings({
      hooks: {
        Stop: [
          {
            matcher: "",
            hooks: [{ type: "command", command: 'node "/old/path/mr-krabs-hook.js" Stop' }],
          },
        ],
      },
    });
    ...
    const settings = readSettings(settingsPath);
    const commands = getMrKrabsCommands(settings, "Stop");
    assert.strictEqual(result.updated, 1);
    assert.strictEqual(commands.length, 1);
    assert.ok(commands[0].includes('hooks/mr-krabs-hook.js'));
