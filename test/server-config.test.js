...
function makeTempHome() {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "mr-krabs-server-config-"));
...
describe("server-config helpers", () => {
  it("clearRuntimeConfig removes runtime.json when present", () => {
    ...
    const runtimeDir = path.join(tmpHome, ".mr-krabs");
    fs.mkdirSync(runtimeDir, { recursive: true });
    const runtimePath = path.join(runtimeDir, "runtime.json");
    fs.writeFileSync(runtimePath, JSON.stringify({ app: "mr-krabs", port: 23333 }));
  ...
  it("probePort recognizes signed Mr. Krabs responses", async () => {
    await new Promise((resolve, reject) => {
      ...
      serverConfig.probePort(23337, 100, (ok) => {
        ...
        httpGet(_options, onResponse) {
          const res = {
            headers: { "x-mr-krabs-server": "mr-krabs" },
            setEncoding() {},
            on(event, handler) {
