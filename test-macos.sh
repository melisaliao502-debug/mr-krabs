#!/bin/bash
# macOS Adaptation Test Script for Mr. Krabs
...
detect_port() {
  local candidates=()
  local runtime_file="$HOME/.mr-krabs/runtime.json"
  ...
  for port in "${candidates[@]}"; do
    if curl -s -D - -o /dev/null --connect-timeout 1 "http://127.0.0.1:$port/state" 2>/dev/null | tr -d '\r' | grep -iq '^x-mr-krabs-server: mr-krabs$'; then
      echo "$port"
      return 0
    fi
  done
...
if [ -z "$PORT" ]; then
  fail "Mr. Krabs not found on ports 23333-23337. Start the app first: npm start"
  exit 1
fi
pass "Mr. Krabs HTTP server is reachable on port $PORT"
...
if echo "$ACCESSIBILITY" | grep -qi "not allowed\|assistive\|1002"; then
  ...
  info "Add your terminal app (or Mr. Krabs) to the allowed list"
else
...
echo ""
echo -e "  ${YELLOW}>>> In 3 seconds, Mr. Krabs will try to focus THIS terminal <<<${RESET}"
...
echo -e "  1. Accessibility permission for your terminal/Mr. Krabs"
